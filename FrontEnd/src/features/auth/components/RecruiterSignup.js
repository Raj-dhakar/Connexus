...
function RecruiterSignup() {
    ...
    const signUp = async () => {
        ...
        try {
            ...
            const response = await authApi.signupRecruiter(recruiterData);
            ...
            if (error.response && error.response.status === 400) {
                toast.error(error.response.data.message || "Signup failed. Account might already exist.")
            } else {
        ...
        <div>
            <Grid container>
                <Grid item xs={6} sx={{ paddingLeft: "80px", paddingTop: "15px" }}>
                    ...
                    <h2 style={{ fontWeight: "100", fontSize: "40px", color: "#B26F28" }}>Join as a Recruiter</h2>
                    ...
                    <label style={{ color: "grey", fontSize: "10px" }}>Company Name</label>
                    <br />
                    <TextField onChange={(e) => setCompanyName(e.target.value)} variant='outlined' label="Company Name" sx={{ width: "400px", mt: "5px", mb: "10px" }} />
                    <br />
