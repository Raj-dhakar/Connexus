...
function RightBar() {
    ...
    const getNews = () => {
        // Note: Public API keys in client-side code are not secure for production
    ...
    const newsList = news ? news.slice(0, 6) : []


    return (
        <div style={{ borderRadius: "10px", backgroundColor: "white", height: "400px", paddingTop: "5px", paddingLeft: "20px" }}>
            <h4>News</h4>
            {newsList.map((eachNews, index) => {
                return <li key={index} style={{ marginTop: "10px" }}>{eachNews.title}</li>
            })}
        </div>
    )
}
